...
const http$1 = require('@graphql-mesh/http');
...
function validateConfig(config, filepath, initialLoggerPrefix, throwOnInvalidConfig = false) {
    ...
    if (!isValid) {
        if (throwOnInvalidConfig) {
            const aggregateError = new utils$1.AggregateError(ajv.errors.map(e => {
                    ...
                    plugin: async () => {
                        ...
                        const results = await Promise.all(rawSources.map(async (source) => {
                            ...
                            if (codeAst) {
                                ...
                                await utils.writeFile(crossHelpers.path.join(artifactsDir, `sources/${source.name}/types.ts`), content);
                            }
                            if (identifier) {
...
async function serveMesh({ baseDir, argsPort, getBuiltMesh, logger, rawServeConfig = {}, playgroundTitle }, cliParams) {
    ...
    if (!cluster.isWorker && Boolean(fork)) {
        ...
        let httpServer;
        const requestHandler = http$1.createMeshHTTPHandler({
            baseDir,
            getBuiltMesh: () => mesh$,
            rawServeConfig,
            playgroundTitle,
        ...
        if (sslCredentials) {
            ...
            httpServer = https.createServer({ key, cert }, requestHandler);
        }
        else {
...
async function graphqlMesh(cliParams = DEFAULT_CLI_PARAMS, args = helpers.hideBin(crossHelpers.process.argv), cwdPath = crossHelpers.process.cwd()) {
    ...
    return yargs(args)
        ...
        try {
            ...
            meshInstance$.then(({ schema }) => utils.writeFile(crossHelpers.path.join(outputDir, 'schema.graphql'), utils$1.printSchemaWithDirectives(schema)).catch(e => logger.error(`An error occured while writing the schema file: `, e)));
            // eslint-disable-next-line @typescript-eslint/no-floating-promises
